...
void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/sensores"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/servo"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
